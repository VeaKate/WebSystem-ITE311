...
class Home extends BaseController // Extend the base controller class
{
    ...
    public function course_management() {
        ...
        return view('admin/course_management'); // Load and return the course management view
    }
    ...
    public function delete($id) {
        ...
        if($this->request->is('post')) {
            ...
            if ($user) {
                $builder->where('id', $id)->delete(); // Delete the user
                session()->setFlashdata('success', 'User deleted successfully.'); // Set success message
            } else {
    ...
    public function edit($id) {
        ...
        $validationRules = [
            ...
            'password' => [
                'label'  => 'Password',
                'rules'  => 'permit_empty|min_length[8]|max_length[255]|regex_match[/^(?!.*[\*"]).+$/]', // Password must be valid and not contain certain symbols
                'errors' => [
                    'regex_match' => 'The {field} must not contain the symbols * or ".'
                ]
            ],
